...
@Composable
fun Leaderboard(gameId: String, gameViewModel: GameViewModel = viewModel()) {
    val leaderboardState by gameViewModel.leaderboard.collectAsState()

    LaunchedEffect(gameId) {
        gameViewModel.fetchLeaderboard(gameId)
    }
        ...
        when (val state = leaderboardState) {
            ...
            is UiState.Success -> {
                ...
                if (leaderboard.isEmpty()) {
                    Text("No rankings yet.", color = LightText)
                } else {
                    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
                        ...
                        LazyColumn {
                            items(leaderboard) { ranking ->
                                Text("${ranking.playerid}: ${ranking.XPpoints} XP")
                            }
                        }
                    }
                }
